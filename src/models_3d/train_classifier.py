...
from src.models_3d.models import get_pretrained_classifier, get_pretrained_encoder
...
def main(config, split, gpu_id, oversample):
    ...
    model = get_pretrained_classifier(path=model_path, encoder_trainable=True)
    model.compile(
    ...
    dir_name = ("classifier__model_genesis__" +
                f"split_{split}__ovrsmpl_{oversample}__" +
